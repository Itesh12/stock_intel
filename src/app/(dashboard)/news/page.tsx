...
export default function NewsIntelligencePage() {
    ...
    const fetchNews = async (isInitial = false) => {
        ...
        try {
            const currentOffset = isInitial ? 0 : offset + COUNT;
            const res = await fetch(`/api/news?count=${COUNT}&offset=${currentOffset}`);
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-700 max-w-[1400px] mx-auto px-4 sm:px-6 py-6 pb-24">
                ...
                {aggregate && (
                    ...
                    >
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="text-xl font-bold text-white tracking-tight">AI Summary</div>
                            <div className="text-[10px] text-slate-500 font-medium">Agreement: {(aggregate.confidence || 0).toFixed(1)}%</div>
                        </div>
                    </motion.div>
                )}
            </div>
            ...
            {filteredNews.length === 0 && (
                <div className="py-20 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-slate-500 text-sm max-w-sm">We couldn't find any news matching your search or filters.</p>
                </div>
            )}
        </div>
    );
