...
export default function NewsIntelligenceClient({ 
    initialNews, 
    initialAggregate, 
    initialHasMore 
}: { 
    initialNews: any[], 
    initialAggregate: any, 
    initialHasMore: boolean 
}) {
    const [newsState, setNewsState] = useState<Record<string, { items: any[], offset: number, hasMore: boolean }>>({
        ALL: { items: initialNews, offset: 0, hasMore: initialHasMore },
        BULLISH: { items: [], offset: 0, hasMore: true },
        BEARISH: { items: [], offset: 0, hasMore: true }
    });
    
    const [aggregate, setAggregate] = useState<any>(initialAggregate);
    ...
    // Refetch when date changes (skip initial mount since we have SSR data)
